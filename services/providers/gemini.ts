...
const generateArtisticPromptForImage = async (symptoms: string, language: string): Promise<string> => {
    ...
    try {
        ...
        return response.text?.trim() || "A tranquil, abstract image with soft colors and gentle light.";
    } catch (error) {
...
Keep the entire response concise, under 150 words. Do not wrap your response in markdown code fences.`;
    ...
    try {
        ...
        return response.text?.trim() || "I'm here to support you through this.";
...
export const getForYouSuggestion = async (profile: UserProfile, language: string): Promise<string> => {
    ...
    try {
        ...
        return response.text?.trim() || "Take a moment to breathe and be kind to yourself.";
...
Generate challenging questions based on the above.`;
    ...
    try {
        ...
        return response.text?.trim() || "- What is another way to look at this situation?\n- If a friend were in this situation, what would you tell them?";
